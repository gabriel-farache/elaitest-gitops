...
Before applying the manifests, there is a need to replace the `__REPLACE_SSH_PRIVATE_KEY__` string in elaitest-argocd-repo.yaml with the SSH key to enable access to the Git repository.

Either manually edit the file or login to the target cluster and run the following command:

```
git clone https://github.com/gabriel-farache/elaitest-gitops.git
cd elaitest-gitops/bootstrap
...
sed -i "s/__REPLACE_SSH_PRIVATE_KEY__/$SSH_PRIVATE_KEY/" elaitest-argocd-repo.yaml
...
```
git clone https://<PAT>@github.com/gabriel-farache/elaitest-gitops.git
```
